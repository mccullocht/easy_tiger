...
enum Golden {
    /// Flat rows of 32-bit neighbor identifiers
    Mapped(DerefVectorStore<u32, Mmap>),
...
impl Golden {
    ...
    fn row(&self, query_index: usize, k: usize) -> Vec<i64> {
        match self {
            Self::Mapped(neighbors) => neighbors[query_index]
                ...
                .map(|x| x as i64)
                .collect(),
            Self::Memory(neighbors) => neighbors[query_index]
                .iter()
                .copied()
                .take(k)
                .map(|n| n.vertex())
...
impl RecallComputer {
    ...
    pub fn in_memory(metric: RecallMetric, k: NonZero<usize>, golden: Vec<Vec<Neighbor>>) -> Self {
        Self {
            metric,
            k: k.get(),
    ...
    fn expected(&self, query_index: usize) -> Vec<i64> {
