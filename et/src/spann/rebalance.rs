...
impl Rebalancer {
    ...
    fn update_head_index(
        &mut self,
        centroids: &[usize],
        clusters: &impl VectorStore<Elem = f32>,
    ) -> Result<()> {
        for (id, vector) in centroids.iter().zip(clusters.iter()) {
            self.head_mutator.update(*id as i64, vector)?;
        }

        for id in centroids.iter().skip(clusters.len()) {
            self.head_mutator.delete(*id as i64)?;
        }

        for vector in clusters.iter().skip(centroids.len()) {
            self.head_mutator.insert(vector)?;
        }

        Ok(())
    }

    fn update_tail_index(
        &self,
        keys: &[PostingKey],
        vectors: &impl VectorStore<Elem = f32>,
    ) -> Result<()> {
        let mut posting_cursor = self
            .head_mutator
            .session()
        ...
        for (old_key, vector) in keys.iter().zip(vectors.iter()) {
            let posting_vector = posting_cursor
                .seek_exact(*old_key)
                .unwrap_or(Err(Error::not_found_error()))?;
            posting_cursor.remove(*old_key)?;

            let candidates = searcher.search(vector, self.head_mutator.reader())?;
            let new_key = PostingKey::new(candidates[0].vertex() as u32, old_key.record_id);
            posting_cursor.set(new_key, &posting_vector)?;
            centroid_cursor.set(
                new_key.record_id,
                new_key.centroid_id.to_le_bytes().as_slice(),
            )?;
        }
...
pub fn rebalance(
    ...
    rebalancer.update_head_index(&centroids, &clusters)?;

    println!("Updated clusters {} => {}", centroids.len(), clusters.len());

    rebalancer.update_tail_index(&keys, &vectors)?;

    println!("Updated tail index");
