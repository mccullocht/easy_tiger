...
// NB: this only handles up to 8k dimensions.
EXPORT uint32_t et_lvq_dot_u1(const uint8_t* a, const uint8_t* b, size_t len) {
  size_t tail = len & ~31;
  uint8x16_t dot0 = vdupq_n_u8(0);
  uint8x16_t dot1 = vdupq_n_u8(0);
  for (size_t i = 0; i < tail; i += 32) {
    uint8x16_t av = vld1q_u8(a + i);
    uint8x16_t bv = vld1q_u8(b + i);
    dot0 = vaddq_u8(dot0, vcntq_u8(vandq_u8(av, bv)));

    av = vld1q_u8(a + i + 16);
    bv = vld1q_u8(b + i + 16);
    dot1 = vaddq_u8(dot1, vcntq_u8(vandq_u8(av, bv)));
  }

  uint32_t dot = vaddlvq_u8(dot0) + vaddlvq_u8(dot1);
