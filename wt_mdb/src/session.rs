...
///
/// Two notable feature of this API are:
...
impl<'a> TransactionGuard<'a> {
    ...
    ///
    /// Note that droping the guard will also rollback the transaction, although you may not specify
...
impl<'a> Drop for TransactionGuard<'a> {
    fn drop(&mut self) {
        ...
        if let Err(e) = self.session.rollback_transaction(None) {
